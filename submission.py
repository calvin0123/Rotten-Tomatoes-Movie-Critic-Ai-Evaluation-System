...
def learnPredictor(trainExamples: List[Tuple[Any, int]], validationExamples: List[Tuple[Any, int]],
    ...
    def predictor(x):
        ans = 1 if dotProduct(featureExtractor(x), weights) > 0 else -1
        return ans
...
def generateDataset(numExamples: int, weights: WeightVector) -> List[Example]:
    ...
    def generateExample() -> Tuple[Dict[str, int], int]:
        ...
        # generate an example -> random choose the word and word's phi from the weights's dict
        # first random choose how many words -> lens / second random choose the word from weights's dict-> random choice
        phi = {}
