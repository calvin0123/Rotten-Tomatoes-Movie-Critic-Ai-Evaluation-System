...
def increment(d1, scale, d2):
    ...
    # first know d2[key] -> find if d1 has that key -> YES : find d1[key] and add d2[key] into that key / NO : get 0
...
def dotProduct(d1, d2):
    ...
    if len(d1) < len(d2):
        ...
        # NOTE: be careful that words is not same in both dictionary, so first find the same word in short dict
        #       and dot them into long dict

        # (ans 1)
        # total = 0
        # for key in d2:
        #     value = d1.get(key, 0)   # get d1's value
        #     total += d2[key] * value  # dot it to d2 -> calculate its total
        # # print(total)
        # return total
